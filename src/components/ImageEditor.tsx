...
const ImageEditor = ({ onClose, previewURL, settings, setSettings }: { onClose: () => void; previewURL: string; settings: { type: "original" | "wide" | "square"; sensitive: boolean }; setSettings: React.Dispatch<React.SetStateAction<{ type: "original" | "wide" | "square"; sensitive: boolean }>> }) => {
    const handleChangeSensitive = (sensitive: boolean) => setSettings((prev) => ({ ...prev, sensitive }));
    const handleChangeType = (type: "original" | "wide" | "square") => setSettings((prev) => ({ ...prev, type }));
    return (
        <div className="fixed w-screen h-screen left-0 top-0 bg-black/75 z-10 flex items-center justify-center">
            <div className="bg-black rounded-xl p-12 flex flex-col gap-4">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <button className="py-2 px-4 rounded-full bg-white text-black font-bold cursor-pointer" onClick={onClose}>
                        Save
                    </button>
                </div>
                {/* IMAGE */}
                <div className="w-[600px] h-[600px] flex items-center">
                    <Image src={previewURL} alt="Preview" width={600} height={600} className={`w-full ${settings.type === "original" ? "h-full object-contain" : settings.type === "square" ? "aspect-square object-cover" : "aspect-video object-cover"}`} />
                </div>
                {/* SETTINGS */}
